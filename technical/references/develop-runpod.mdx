...
2. Select an appropriate GPU (ex: RTX 4090) and make the following changes:
    ...
    2. Click **Change Template**, select `Torch 2.4.0` <Note>Alternatively, you can choose this [RunPod template](https://runpod.io/console/deploy?template=6dj77fsqa8&ref=u8tlskew) which is pre-configured with the settings below. For ComfyStream development, consider using the [ComfyStream template](https://runpod.io/console/deploy?template=w01m180vxx&ref=u8tlskew) which includes the new `--use-volume` flag for improved performance.</Note>
