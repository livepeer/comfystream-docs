...
<Steps>
  ...
  <Step title="Run the container">
    ...
    <Info>
      For improved performance with host volume mounting, you can add the `--use-volume` flag to the command above. This flag enables more performant volume mounting and is particularly beneficial in cloud environments.
    </Info>
  </Step>
...
<Steps>
  <Step title="Launch the RunPod template">
    ...
    <Note>
      The template now uses the new `--use-volume` flag for improved performance with host volume mounting. This replaces the previous two separate templates (with and without volume support) with a single, more performant solution.
    </Note>
...
#### Understanding the `--use-volume` Flag

The `--use-volume` flag is a new feature that improves performance when using host volume mounting, particularly in cloud environments like RunPod. Here's what you need to know:

<AccordionGroup>
  <Accordion title="What does `--use-volume` do?">
    The `--use-volume` flag enables more performant host volume mounting by optimizing how ComfyStream interacts with mounted volumes. This is especially beneficial when:
    - Running in cloud environments with network-attached storage
    - Using persistent volumes for model storage
    - Working with large model files that need frequent access
  </Accordion>

  <Accordion title="When should I use it?">
    You should use the `--use-volume` flag when:
    - Deploying on RunPod or other cloud platforms
    - Using persistent volumes for data storage
    - Experiencing slow model loading or file access
    - Working with large workflows that require frequent model access
  </Accordion>

  <Accordion title="How to add it to your command">
    Simply add `--use-volume` to your existing Docker run command:
    ```bash
    docker run -it --gpus all \
      -p 8188:8188 \
      -p 8889:8889 \
      ...
      -v ~/models/ComfyUI--models:/workspace/ComfyUI/models \
      -v ~/models/ComfyUI--output:/workspace/ComfyUI/output \
      livepeer/comfystream:stable --download-models --build-engines --server --use-volume
    ```
  </Accordion>
</AccordionGroup>
...
<Steps>
  ...
  <Step title="Run the playbook">
    ...
    <Info>
      The Ansible playbook automatically includes the `--use-volume` flag for improved performance with cloud storage.
    </Info>
  </Step>
