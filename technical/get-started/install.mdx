...
<Steps>
  ...
  <Step title="Run the container">
    ...
    docker run -it --gpus all \
      ...
      livepeer/comfystream:latest --download-models --build-engines --api --ui
    ```
    <Note>
      Available flags:
      ...
      - `--use-volume` enables more performant volume mounting (recommended for cloud environments)
...
<Steps>
  <Step title="Launch the RunPod template">
    ...
    [livepeer-comfystream](https://console.runpod.io/deploy?template=p6j5o97hbw&ref=u8tlskew)
    <Note>
      This template includes the ComfyStream UI and uses the `--use-volume` flag for improved performance. A network volume is recommended for persistence across pod restarts.
    </Note>
