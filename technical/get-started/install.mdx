...
<Steps>
  <Step title="Install ComfyUI (if needed)">
    ...
    <Note>
      ComfyUI must be run with frontend version `v1.24.2`. You can do this by launching ComfyUI with the `--front-end-version v1.24.2` flag.
    </Note>
  </Step>
...
<Accordion title="Manual Installation (Cloning Repository)">
  ...
  ```bash
  cd ComfyUI
  python main.py --listen --front-end-version v1.24.2
  ```
